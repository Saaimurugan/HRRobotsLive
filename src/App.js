...
const Header = () => {
  ...
  const handleNavBtnMouseLeave = (e) => {
    e.currentTarget.style.background = "transparent";
    // Reset SVG icon color
    const paths = e.currentTarget.querySelectorAll('path');
    paths.forEach(path => {
      if (path.getAttribute('stroke')) {
        path.setAttribute('stroke', '#1C274C');
      }
      if (path.getAttribute('fill') && path.getAttribute('fill') !== 'none') {
            ...
            >
              ...
              {showProfilePopup && globalValue && (
                <div role="tooltip" style={{
                  ...
                  <div style={{ fontWeight: "600", marginBottom: "4px", color: "#374151" }}>Logged in as</div>
                  <div style={{ color: "#475569" }}>{globalValue}</div>
                </div>
              )}
            </div>
            <button className="nav-btn" style={navButtonStyle} onClick={() => navigate("/logout")} onMouseEnter={handleNavBtnMouseEnter} onMouseLeave={handleNavBtnMouseLeave} aria-label="Log out" title="Log out">
              <svg width="30px" height="30px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">
