...
const ResetPage = lazy(() => import('./components/reset'));
...
const FaceDetection = lazy(() => import('./components/faceDetection'));
  ...
  return (
    <header
      ...
      style={{
        ...
        top: 0,
        width: "100%",
        background: isScrolled ? "rgba(28, 187, 180, 0.7)" : "#1CBBB4",
        color: "white",
        padding: "0px",
        ...
        boxShadow: isScrolled ? "none" : "0 4px 8px rgba(0, 0, 0, 0.2)",
        zIndex: 1000,
        transition: "all 0.3s ease"
      }}
    >
      <img
        src="../logo.png"
        alt="HRRobots - AI Interview Platform"
        style={{ padding: "5px", height: "50px", marginRight: "0px" }}
      />
      <nav role="navigation" aria-label="Main navigation" style={{ display: "flex", alignItems: "center", paddingRight: "15px", gap: "5px" }}>
        {isResult ? (
          <>
            {!isLogin && (
              <button style={navButtonStyle} onClick={() => navigate("/list")} aria-label="Go to Dashboard" title="Dashboard">
          ...
          <>
            ...
            >
              <button style={navButtonStyle} onClick={() => navigate("/profile")} aria-label="View Profile" aria-expanded={showProfilePopup} title="Profile">
                <svg width="30px" height="30px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">
              ...
              {showProfilePopup && globalValue && (
                <div role="tooltip" style={{
                  ...
                  <div style={{ fontWeight: "600", marginBottom: "4px", color: "#1CBBB4" }}>Logged in as</div>
                  <div style={{ color: "#555" }}>{globalValue}</div>
                </div>
              )}
            </div>
            <button style={navButtonStyle} onClick={() => navigate("/logout")} aria-label="Log out" title="Log out">
              <svg width="30px" height="30px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">
...
const App = () => {
  // Preload face-api.js models when the app starts
  // Remove blocking model preloading from app startup
  // Models will be loaded lazily when face detection is needed
  useEffect(() => {
        ...
        <Router>
          <TourProvider>
            <>
              ...
              <div aria-live="polite" aria-atomic="true" className="aria-live-region" id="aria-live-region"></div>
              <Header />
